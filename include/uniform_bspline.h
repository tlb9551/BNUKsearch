...
class UniformBSpline {
 public:
  ...
  _Scalar evaluate(_Scalar t, int derivative, int &s_i) const {
    ...
    s_i = s;
    _Scalar u = s - s_i;
    if (s_i > control_points_.size() - _N || s_i < 0) {
      std::cerr << "evalute():Curve is not defined at this time point :  " << t << std::endl;
      return 0;
    }
    VectorN coefficients;
    for (int i = 0; i < _N; i++)
      //coefficients[i] = control_points_[clampIdx(s_i + i - OFFSET)];
      coefficients[i] = control_points_[clampIdx(s_i + i)];
  ...
  _Scalar quadraticCost(const MatrixN &quadratic_cost, int cp_start_idx = 0,
                        int num_points = -1) const {//num_points 从1开始。1即 一段
    ...
    int min_idx = std::max(minValidIdx(), cp_start_idx);
    int max_idx = std::min(maxValidIdx(), cp_start_idx + num_points - 1);

    for (int idx = min_idx; idx < max_idx + 1; ++idx) {
      VectorN coefficients;
      for (int i = 0; i < _N; i++)
        //coefficients[i] = control_points_[idx + i - OFFSET];
        coefficients[i] = control_points_[idx + i];
      _Scalar cost =
...
//    res *= pow_inv_dt_[2 * deriv - 1];
  ...
  void addCoeffs(const Eigen::MatrixBase<Derived> &coeffs_const) {
  ...
  inline int maxValidIdx() const {
    ...
    return control_points_.size() - N + 1;
  }
...
//  inline int openIdx(int idx) const
//  {
//      int idx1 = std::min(idx,static_cast<int>(control_points_.size()) - 1);
//      return std::max(idx1,0);
//  }
...
template<int _N, typename _Scalar>
typename UniformBSpline<_N, _Scalar>::MatrixNArray computeQuadraticCostJacobian() {
  typename UniformBSpline<_N, _Scalar>::MatrixNArray m_array;
  typename UniformBSpline<_N, double>::MatrixN blending_matrix = computeBlendingMatrix<_N, double>();
