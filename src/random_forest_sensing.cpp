...
void ReadMap() {
  stringstream map_dir;
  map_dir << "/home/tlb/catkin_test/src/BNUK_Opt/data/50_50_5/map" << map_count << ".pcd";
  if (pcl::io::loadPCDFile<pcl::PointXYZ>(map_dir.str(), cloudMap) == -1) //* load the file
  {
    PCL_ERROR("Couldn't read file map.pcd \n");
    return;
  }
  if ((int) cloudMap.points.size() == 0)
    return;
...
int main(int argc, char **argv) {
  ...
  n.param("map_count", map_count, 1);
  n.param("read_map", read_map, true);
