...
class TrajectoryServer {
 ...
 public:
  ...
  void rcvOdometryCallback(const nav_msgs::Odometry &odom) {
    ...
    if (state == TRAJ
        && ((odom.header.stamp - _start_time).toSec() / mag_coeff > (_final_time - _start_time).toSec())) {
      state = HOVER;
      ...
      for (int idx = 0; idx < _n_segment; ++idx) {
        if (t > _time[idx] && idx + 1 < _n_segment) {
          ...
          for (int i = 0; i < cur_poly_num; i++) {
            _cmd.position.x += _time[idx] * CList[cur_order](i) * _coef[_DIM_x].col(idx)(i) * pow(t, i)
                * pow((1 - t), (cur_order - i));
            _cmd.position.y += _time[idx] * CList[cur_order](i) * _coef[_DIM_y].col(idx)(i) * pow(t, i)
                * pow((1 - t), (cur_order - i));
            _cmd.position.z += _time[idx] * CList[cur_order](i) * _coef[_DIM_z].col(idx)(i) * pow(t, i)
                * pow((1 - t), (cur_order - i));

            if (i < (cur_poly_num - 1)) {
              _cmd.velocity.x +=
                  CvList[cur_order](i) * cur_order * (_coef[_DIM_x].col(idx)(i + 1) - _coef[_DIM_x].col(idx)(i))
                      * pow(t, i) * pow((1 - t), (cur_order - 1 - i));

              _cmd.velocity.y +=
                  CvList[cur_order](i) * cur_order * (_coef[_DIM_y].col(idx)(i + 1) - _coef[_DIM_y].col(idx)(i))
                      * pow(t, i) * pow((1 - t), (cur_order - 1 - i));

              _cmd.velocity.z +=
                  CvList[cur_order](i) * cur_order * (_coef[_DIM_z].col(idx)(i + 1) - _coef[_DIM_z].col(idx)(i))
                      * pow(t, i) * pow((1 - t), (cur_order - 1 - i));
            }
