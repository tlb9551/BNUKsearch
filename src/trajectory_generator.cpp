...
#define ENFORCE_ACC  isLimitAcc // whether or not adding extra constraints for ensuring the acceleration feasibility
  ...
  int equ_con_num = equ_con_s_num + equ_con_e_num
      + equ_con_continuity_num; // p, v, a in x, y, z axis in each segment's joint position
  ...
  for (int i = 0; i < equ_con_num; i++) {
    ...
    pair<MSKboundkeye, pair<double, double> > cb_eq =
        make_pair(MSK_BK_FX, make_pair(beq_i, beq_i)); // # cb_eq means: constriants boundary of equality constrain
  ...
  for (int k = 0; k < segment_num; k++) {
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < n_poly; j++) {
        ...
        vb_x = make_pair(MSK_BK_RA,
                         make_pair(lo_bound,
                                   up_bound)); // # vb_x means: varialbles boundary of unknowns x (Polynomial coeff)
